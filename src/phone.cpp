...
void help()
{
    cout << endl << "USAGE: ./phone IMAGE.EXTENTION checkcas.xml cas1.xml cas2.xml cas3.xml cas4.xml ..........upto n number of main cascade xml files" << endl;
        ...
        for( vector<Rect>::const_iterator main = cars.begin(); main != cars.end(); main++, i++ )
        {
            ...
            for( vector<Rect>::const_iterator sub = nestedcars.begin(); sub != nestedcars.end(); sub++ )      //testing the detected car by main using checkcascade
            {
                ...
                if(cen_x>(x0+15) && cen_x<(x1-15) && cen_y>(y0+15) && cen_y<(y1-15))         //if centre of bounding circle is inside the rectangle boundary over a threshold the the car is certified
                {
                    ...
                    //cout << endl << "x0: " << x0 << "   y0: " << y0 << "   x1: " << x1 << "   y1: " << y1 << endl;

                    //for (vector<Rect>::const_iterator igloo = nestedcars.begin(); igloo != nestedcars.end(); ++igloo)
                      //  cout << *igloo << ' ';
                    //cout << endl;
                    ...
                    myvector.push_back(region_of_interest);                 // puts rectangle into a vector
            ...
            cout << "rect" << endl;
            for (vector<Rect>::const_iterator igloo = myvector.begin(); igloo != myvector.end(); ++igloo)
                cout << *igloo << ' ';
            cout << endl;
...
int main( int argc, const char** argv )
{
    ...
    if(detectcars.num!=0)
    {
        cout << endl << detectcars.num << " phones got detected in = " << t/((double)cvGetTickFrequency()*1000.) << " ms" << endl << endl;
    }
    else
    {
        cout << endl << "phone not found" << endl;
    }
